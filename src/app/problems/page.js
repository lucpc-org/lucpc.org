  ...
  const completeProblem = (e) => {
    ...
    Promise.all([get(userRef)]).then((snapshots) => {
      if (snapshots[0].exists()) {
        ...
        problems[e.target.id][weekDate] = [
          e.target.checked ? today.toISOString() : null,
        ];

        let order = {};
        order[weekDate + "-" + e.target.id] = [
          e.target.checked ? today.toISOString() : null,
        ];
              ...
              <div>
                <div className="w-fit h-fit">
                  <h1
                    className={
                      "bg-" +
                      item.difficulty +
                      " hidden sm:flex h-fit w-fit flex-row justify-center font-bold text-base md:text-lg xl:text-2xl rounded-lg p-2 sm:px-4 mx-auto"
                    }
                  >
                    {item.difficulty}
                  </h1>
                ...
                <div
                  className={
                    "bg-" +
                    item.difficulty +
                    " flex sm:hidden h-10 rounded-lg p-2 sm:px-4 mx-auto"
                  }
                ></div>
              </div>
              <h1 className="font-bold text-sm lg:text-lg xl:text-xl mr-auto w-fit px-4 sm:px-8">
                {item.name}
              </h1>
              <a target="_blank" rel="noreferrer" href={item.url}>
                <div className="mx-auto bg-neutral-200 hover:bg-neutral-500 text-center text-shadow font-bold w-fit p-2 rounded-lg">
                  <svg
                    className="h-6"
              ...
              {!(currentUser === null || currentUser === undefined) && (
                <>
                  <input
                    ...
                    id={item.difficulty}
                    value=""
                    defaultChecked={solvedStates[item.difficulty]}
                    className="hidden peer"
                  />
                  <label
                    htmlFor={item.difficulty}
                    className="text-neutral-500 hover:text-neutral-800 peer-checked:text-easy cursor-pointer"
                  >
                    <svg
                      className="h-8 lg:h-10 p-1 pl-3"
                      fill="currentColor"
                      aria-hidden="true"
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
